...
import 'package:dart_application_1/models/service/log.dart';
import 'package:dart_application_1/utils/resp.dart';
...
class AppOperationController extends ResourceController {
  ...
  Future<Response> getMarks(
      ...
      {@Bind.path("page") int page = 0}) async {
  ...
  Future<Response> searchMark(
      @Bind.header(HttpHeaders.authorizationHeader) String header,
      {@Bind.query('name') String? markName,
    ...
    var currentOperations = await (Query<Marks>(context)
          ...
          ..where((x) => x.isntDeleted).equalTo(isHidden == null)
          ..sortBy((x) => x.createDate, QuerySortOrder.descending))
        .fetch();
    if (currentOperations.isEmpty) {
      return Response.noContent();
